...
Timeseries <- function(obslabel,tsdata,varname,ytext,legendtext,
                       ...
                       vqcdata=matrix(-1,nrow=1,ncol=1),na.rm=FALSE, x_axis=TRUE){
  #
  errtext = 'ok'
  metrics = list()
  ...
  if(smoothed){
    ...
    if(all(is.na(data_smooth[,1]))){
      ...
      plot(xloc,data_smooth[,1],type="l",ylab=ytext,lwd=3,
           col=plotcolours[1],ylim=c((ymin),(ymin + (ymax-ymin)*1.05)),
    ...
    if(all(is.na(tsdata[,1]))){
      ...
      plot(xloc,tsdata[,1],type="l",ylab=ytext,lwd=3,
           col=plotcolours[1],ylim=c(ymin,(ymin + (ymax-ymin)*1.1)),
           xaxt='n',cex.lab=plotcex,cex.axis=plotcex,xlab='',mgp = c(2.5+plotcex*0.9,0.8,0))
      ...
      if(na.rm){
        ...
        if(vqcdata[1,1] != -1){
          ...
          text(x=stattextx[1],y=qctexty,cex=max((plotcex*0.95),1),pos=4,
               labels=paste(qcpc,'% gap-filled:',sep=''))
        }
      }
    } #all NA?
    for(l in 1:nyears){
    ...
    if(x_axis) { axis(1,at=xxat,labels=xxlab,cex.axis=plotcex,mgp = c(2.3,plotcex*0.7,0)) }
